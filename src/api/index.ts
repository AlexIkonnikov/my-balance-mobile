...
export const signUpRequest = (data: UserCreateRequest): AxiosPromise<UserCreateResponce> => {
  return HttpService.post(ENDPOINTS.AUTH_REGISTRATION, { data });
};

export const signInRequest = (data: UserLoginRequest): AxiosPromise<UserCreateResponce> => {
  return HttpService.post(ENDPOINTS.AUTH_LOGIN, { data });
};

export const getTransactionPerMounth = async (start: string, end: string): Promise<TransactionResponse> => {
  const { data } = await HttpService.get(`${ENDPOINTS.TRANSCTIONS}/${start}/${end}`);
  return data;
};

export const deleteTransaction = (id: number): Promise<AxiosResponse<Transaction>> => {
